...
export function Sidebar() {
  ...
  const [username, setUsername] = useState("Loading..."); // Default state for username

  useEffect(() => {
    // Update the username when the session is loaded and available
    if (status === "authenticated" && session?.user) {
      setUsername(session.user.username || "Unknown User");
  ...
  return (
    <div className="fixed left-0 top-0 h-full w-64 bg-[#2C3333] text-white">
      <div className="flex h-16 items-center px-6">
        ...
        <span className="ml-3 text-sm font-semibold">{username}</span>
      </div>
          ...
          <Link
            ...
            className={`flex items-center rounded-lg px-4 py-2 text-sm font-medium ${
              [
                ...
                "/dashboard/dash/team",
                "/dashboard/dash/campaign",
                "/dashboard/dash/client",
                "/dashboard/dash/alerts",
              ].includes(pathname)
                ? "bg-[#5C5C5C] text-white"
                : "text-[#E7E7E7] hover:bg-[#5C5C5C]"
            }`}
